...
def write_returns_to_excel(filepath: str, df: pd.DataFrame) -> None:
    ...
    for idx, row in df.iterrows():
        row_index = idx + 2
        ws.cell(row=row_index, column=5, value=row['Nifty50_Return (%)'])
        ws.cell(row=row_index, column=6, value=row['Sensex_Return (%)'])
        apply_border(ws.cell(row=row_index, column=5))
        apply_border(ws.cell(row=row_index, column=6))
...
def write_summary_statistics_to_excel(filepath: str, df_returns: pd.DataFrame) -> dict:
    ...
    risk_free_rate = 2 / len(df_returns)
    ...
    for i, label in enumerate(labels, start=2):
        ...
        ws.cell(row=i, column=9, value=values_nifty[i - 2])
        ws.cell(row=i, column=10, value=values_sensex[i - 2] if values_sensex[i - 2] != '' else None)
        apply_border(ws.cell(row=i, column=9))
        apply_border(ws.cell(row=i, column=10))
...
def write_portfolio_weights(filepath: str) -> None:
    weights = [(150 - i * 10, -50 + i * 10) for i in range(21)]
    wb = load_workbook(filepath)
    ws = wb['Sheet1']
    ...
    for i, (w1, w2) in enumerate(weights, start=2):
        ws.cell(row=i, column=12, value=w1)
        ws.cell(row=i, column=13, value=w2)
        apply_border(ws.cell(row=i, column=12))
        apply_border(ws.cell(row=i, column=13))
...
def write_portfolio_summary(filepath: str, stats: dict) -> None:
    ...
    for i in range(21):
        ...
        ws.cell(row=row_start + i, column=col_start, value=port_return)
        ws.cell(row=row_start + i, column=col_start + 1, value=port_std)
        ws.cell(row=row_start + i, column=col_start + 2, value=sharpe)
        for col in range(col_start, col_start + 3):
            apply_border(ws.cell(row=row_start + i, column=col))
...
def write_cal(filepath: str, stats: dict) -> None:
    ...
    best_weight = max([(150 - i * 10, -50 + i * 10) for i in range(21)],
    ...
    for i, weight in enumerate([0, 1, 2], start=3):
        ...
        ws.cell(row=i, column=col_start + 1, value=cal_ret)  # Correct: Return
        ws.cell(row=i, column=col_start + 2, value=cal_std)  # Correct: STD
        for col in range(col_start, col_start + 3):
            apply_border(ws.cell(row=i, column=col))
